...
struct TaskPromise {
 ...
 private:
  ...
  std::list<std::function<void(Result<ResultType>)>> completion_cbs;

  Executor executor;

  void notify_cbs() {
    auto value = result.value();
    for (auto &cb : completion_cbs) {
      cb(value);
    }
    completion_cbs.clear();
  }
...
struct TaskPromise<void, Executor> {
 ...
 private:
  ...
  std::list<std::function<void(Result<void>)>> completion_cbs;
